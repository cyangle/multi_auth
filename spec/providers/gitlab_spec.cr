...
describe MultiAuth::Provider::Gitlab do
  ...
  it "generates authorize_uri with pkce params" do
    uri = MultiAuth.make("gitlab", "/callback").authorize_uri(state: "random_state_value") do |form|
      form.add "code_challenge", "code_challenge_value"
      form.add "code_challenge_method", "S256"
    end
    uri.should eq("https://gitlab.com/oauth/authorize?client_id=gitlab_id&redirect_uri=%2Fcallback&response_type=code&scope=&state=random_state_value&code_challenge=code_challenge_value&code_challenge_method=S256")
  end
